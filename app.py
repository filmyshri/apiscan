...
def _face_vector(face, width, height):
    landmarks = {lm.get("type"): lm.get("position", {}) for lm in face.get("landmarks", [])}
    vector = []
    for key in _LANDMARK_ORDER:
        pos = landmarks.get(key, {})
        x = float(pos.get("x", 0.0)) / max(width, 1)
        y = float(pos.get("y", 0.0)) / max(height, 1)
        z = float(pos.get("z", 0.0)) / 1000.0
        vector.extend([x, y, z])
    box = face.get("boundingPoly", {}).get("vertices", [])
    if len(box) >= 2:
        x0 = float(box[0].get("x", 0.0))
        y0 = float(box[0].get("y", 0.0))
        x1 = float(box[2].get("x", x0))
        y1 = float(box[2].get("y", y0))
        vector.extend([x0 / max(width, 1), y0 / max(height, 1)])
        vector.extend([(x1 - x0) / max(width, 1), (y1 - y0) / max(height, 1)])
    else:
...
def _face_distance(encoding_a, encoding_b):
    return sum((a - b) ** 2 for a, b in zip(encoding_a, encoding_b)) ** 0.5
...
@app.route("/events/<event_id>/open-folder", methods=["POST"])
def open_event_folder(event_id):
    ...
    if not event:
        return jsonify(error="Event not found."), 404
    folder_path = _event_folder_base(photographer_id, event_id)
    if not os.path.isdir(folder_path):
        folder_path = _event_photo_dir(photographer_id, event_id)
    if not os.path.isdir(folder_path):
