...
const setSelfieEnabled = (enabled) => {
  selfieEnabled = enabled;
  if (enabled) {
    eventSelfieStatus.textContent = "";
    eventSearchButton.disabled = !eventSelfieFile;
    eventBrowseButton.disabled = false;
  } else {
    eventSelfieStatus.textContent = "Selfie match is disabled for this event.";
    eventSearchButton.disabled = true;
    eventBrowseButton.disabled = true;
    eventFileInput.value = "";
    eventPreviewImage.classList.add("hidden");
  }
};
...
const handleEventFile = (file) => {
  ...
  eventSearchButton.disabled = !selfieEnabled;
...
loadEventButton.addEventListener("click", async () => {
  ...
  try {
    ...
    setSelfieEnabled(Boolean(data.match_enabled));
    loadEventFolders();
    loadEventGallery();
    updateSelectedUI();
  } catch (error) {
