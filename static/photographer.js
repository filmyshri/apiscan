...
document.querySelectorAll(".open-event-folder").forEach((button) => {
  button.addEventListener("click", async (event) => {
    ...
    if (!eventId) {
      eventStatusText.textContent = "Missing event ID.";
      return;
    }
    eventStatusText.textContent = "Opening event folder...";
    try {
      const response = await fetch(`/events/${eventId}/open-folder`, { method: "POST" });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to open folder.");
      }
      eventStatusText.textContent = "Event folder opened.";
    } catch (error) {
      eventStatusText.textContent = error.message;
    }
  });
